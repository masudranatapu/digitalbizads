@extends('layouts.web', ['nav' => true, 'banner' => false, 'footer' => true, 'cookie' => true, 'setting' => true, 'title' => __('Emojies - Web Tools')])
        ...
        <section class="text-gray-700">
            <div class="container px-5 py-24 mx-auto">
                ...
                <div class='mb-3 space-y-2 w-full'>
                    ...
                    <textarea
                        class="content appearance-none block w-full bg-grey-lighter text-grey-darker border border-grey-lighter font-mono text-base text-black rounded-lg h-48 px-4 py-2"
                        id="content" name="content" cols="30" rows="10" placeholder="{{ __('Content') }}"></textarea>
                </div>

                <button
                    class="emojiPicker lg:inline-block py-2 px-6 bg-{{ $config[11]->config_value }}-500 hover:bg-{{ $config[11]->config_value }}-600 text-lg text-white font-bold rounded-l-xl rounded-t-xl transition duration-200 mb-12">{{ __('Start Picker') }}</button>
                ...
                <button
                    ...
                    data-clipboard-action="copy" data-clipboard-target="#content">{{ __('Copy to clipboard') }}</button>
            </div>
        </section>
    </div>
